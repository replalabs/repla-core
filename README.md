...
REPLA is a framework for game builders on Solana. It sits on top of MagicBlock's Ephemeral Rollup primitive and gives you four things:

- A **sequencer runtime** in Rust that wraps the ER session with deterministic batching, replay, and slashing detection.
- An **Anchor settler** program on Solana mainnet that anchors batched state roots through PDA-keyed instructions.
